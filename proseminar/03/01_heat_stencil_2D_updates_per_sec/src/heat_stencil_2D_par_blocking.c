...
void data_to_csv(int problem_size, double time, int num_ranks, double updates_per_sec);
...
int main(int argc, char **argv) {
    ...
    for (int t = 0; t < T; t++) {
        ...
        compute_time += ((double)(clock() - compute_start)) / CLOCKS_PER_SEC;
    ...
    double updates_per_sec = total_updates / compute_time;
    double total_updates_per_sec = 0;
    MPI_Reduce(&updates_per_sec, &total_updates_per_sec, 1, MPI_DOUBLE, MPI_SUM, 0, MPI_COMM_WORLD);
    ...
    if (my_rank == 0)  {
        ...
        data_to_csv(N, total_time, num_ranks, total_updates_per_sec);
...
void data_to_csv(int problem_size, double time, int num_ranks, double updates_per_sec) {
	...
	if(set_header) fprintf(fpt, "Impl/Ranks,Problem Size,Time,Updates/Sec\n");
	fprintf(fpt, "%s/%d,%u,%.9f,%.9f\n", TYPE, num_ranks, problem_size, time, updates_per_sec);
	fclose(fpt);
}