...
# Function to process the input CSV, compute metrics (mean, std, speedup, efficiency, Updates/Sec), and return processed data
def process_data(csv_file):
    ...
    aggregated_df = df.groupby(['Impl/Ranks', 'Problem Size'], sort=False).agg(
        Time_mean=('Time', 'mean'),
        Time_std=('Time', 'std'),
        Calc_sec_mean=('Updates/Sec', 'mean'),
        Calc_sec_std=('Updates/Sec', 'std'),
    ...
    merged_df = merged_df.round({'Time_mean': 2, 'Time_std': 2, 'Speedup': 2, 'Efficiency': 2, 'Calc_sec_mean': 2, 'Calc_sec_std': 2})

    # Reorder the columns to place 'Repetitions' directly after 'Problem Size'
    merged_df = merged_df[['Impl/Ranks', 'Problem Size', 'Repetitions', 'Time_mean', 'Time_std', 'Calc_sec_mean', 'Calc_sec_std', 'Speedup', 'Efficiency']]
...
# Function to generate plots for Wall Time, Speedup, Efficiency, and Updates/Sec based on processed data
def create_plots(processed_csv, repetitions):
    ...
    # Create a Updates/Sec plot
    fig, ax = plt.subplots(figsize=(8, 6))
    sns.barplot(x='Impl/Ranks', y='Calc_sec_mean', hue='Problem Size', data=df, ax=ax, errorbar='sd', palette=colors)
    ax.set_title(f'Updates/Sec (Mean) - {repetitions} Repetitions')
    ax.set_xlabel('Impl/Ranks')
    ax.set_ylabel('Updates/Sec')
    ax.tick_params(axis='x', rotation=45)
    plt.tight_layout()
    plt.savefig(f"{processed_csv.replace('_processed.csv', '_calc_sec.png')}")
    plt.close()
...
if __name__ == '__main__':
    ...
    for csv_file in csv_files:
        ...
        # Step 2: Generate plots for Wall Time, Speedup, Efficiency, and Updates/Sec
