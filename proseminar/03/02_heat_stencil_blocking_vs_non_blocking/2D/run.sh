...
for i in $(seq 1 $REP); do
    for N in $PROBLEMS; do
        ...
        for P in $RANKS; do
            par_block_cmd="mpiexec -n $P build/heat_stencil_2D_par_blocking $N"
            previous_job_id=$(submit_job $P "$previous_job_id" "$par_block_cmd")

            par_non_block_cmd="mpiexec -n $P build/heat_stencil_2D_par_non_blocking $N"
            previous_job_id=$(submit_job $P "$previous_job_id" "$par_non_block_cmd")
        done
    done
done
