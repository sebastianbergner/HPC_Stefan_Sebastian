...
void propagate_heat(int i_start, int i_max, int i_add, Vector A, Vector B, int my_rank, int rank_with_source, int vector_size_per_rank, int source_x, int left_rank, int right_rank, value_t t_left, value_t t_right);
...
int main(int argc, char **argv) {
  ...
  int right_rank = (my_rank != num_ranks-1) ? my_rank+1 : MPI_PROC_NULL;
  ...
  for (int t = 0; t < T; t++) {
    ...
    MPI_Isend(&A[0], 1, MPI_DOUBLE, left_rank, 0, MPI_COMM_WORLD, &requests[1]);
    MPI_Isend(&A[vector_size_per_rank-1], 1, MPI_DOUBLE, right_rank, 0, MPI_COMM_WORLD, &requests[0]);
    MPI_Irecv(&t_left, 1, MPI_DOUBLE, left_rank, 0, MPI_COMM_WORLD, &requests[2]);
    MPI_Irecv(&t_right, 1, MPI_DOUBLE, right_rank, 0, MPI_COMM_WORLD, &requests[3]);

    // .. we propagate the temperature for all possible cases
    int i_start = 1, i_max = (vector_size_per_rank-1), i_add = 1;
    propagate_heat(i_start, i_max, i_add, A, B, my_rank, rank_with_source, vector_size_per_rank, source_x, left_rank, right_rank, t_left, t_right);

    MPI_Waitall(4, requests, MPI_STATUS_IGNORE);

    // ... now propagate heat for edge cases
    i_start = 0, i_max = vector_size_per_rank, i_add = (vector_size_per_rank-1);
    propagate_heat(i_start, i_max, i_add, A, B, my_rank, rank_with_source, vector_size_per_rank, source_x, left_rank, right_rank, t_left, t_right);
...
void propagate_heat(int i_start, int i_max, int i_add, Vector A, Vector B, int my_rank, int rank_with_source, int vector_size_per_rank, int source_x, int left_rank, int right_rank, value_t t_left, value_t t_right) {
  for (int i = i_start; i < i_max; i += i_add) {
    ...
    value_t tl = (i != 0) ? A[i - 1] : (left_rank != MPI_PROC_NULL ? t_left : tc);
    value_t tr = (i != vector_size_per_rank - 1) ? A[i + 1] : (right_rank != MPI_PROC_NULL ? t_right : tc);
