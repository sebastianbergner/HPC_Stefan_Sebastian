...
void gather_data_from_submatrices(Matrix submatrix, Matrix supermatrix, int myRank, int numRanks, int rows_per_rank, int cols_per_rank, int ranks_per_row, int N);
...
int main(int argc, char **argv) {
    ...
    for (int t = 0; t < T; t++) {
        ...
        // TODO: comment
        MPI_Isend(send_up_temps,    cols_per_rank, MPI_DOUBLE, up,    0, MPI_COMM_WORLD, &requests[0]);
        MPI_Isend(send_down_temps,  cols_per_rank, MPI_DOUBLE, down,  0, MPI_COMM_WORLD, &requests[1]);
        MPI_Isend(send_left_temps,  rows_per_rank, MPI_DOUBLE, left,  0, MPI_COMM_WORLD, &requests[2]);
        MPI_Isend(send_right_temps, rows_per_rank, MPI_DOUBLE, right, 0, MPI_COMM_WORLD, &requests[3]);

        MPI_Irecv(recv_up_temps,    cols_per_rank, MPI_DOUBLE, up,    0, MPI_COMM_WORLD, &requests[4]);
        MPI_Irecv(recv_down_temps,  cols_per_rank, MPI_DOUBLE, down,  0, MPI_COMM_WORLD, &requests[5]);
        MPI_Irecv(recv_left_temps,  rows_per_rank, MPI_DOUBLE, left,  0, MPI_COMM_WORLD, &requests[6]);
        MPI_Irecv(recv_right_temps, rows_per_rank, MPI_DOUBLE, right, 0, MPI_COMM_WORLD, &requests[7]);

        

        // .. we propagate the temperature; do all possible calculations here
        for (int i = 1; i < rows_per_rank-1; i++) {
            for (int j = 1; j < cols_per_rank-1; j++) {
        ...
        MPI_Waitall(8, requests, NULL);


        // then the rest of the computations here
        for (int i = 0; i < rows_per_rank; i+= rows_per_rank-1) { // up down
        ...
        for (int i = 0; i < rows_per_rank; i++) { // left right
            for (int j = 0; j < cols_per_rank; j+=cols_per_rank-1) {
                if(myRank == rank_with_source && (i == (source_i % rows_per_rank)) && (j == (source_j % cols_per_rank))){
                    B[calc_index(i, j, cols_per_rank)] = A[calc_index(i, j, cols_per_rank)];
                    continue;
                }
                // if (myRank == 0){
                //     printf("right left loop i %d j %d\n", i, j);
                // }
                // get temperature at current position
                value_t current_temp = A[calc_index(i, j, cols_per_rank)];
        ...
        if (!(t % 10000)) {
            ...
            gather_data_from_submatrices(A, printMat, myRank, numRanks, rows_per_rank, cols_per_rank, ranks_per_row, N);
...
void gather_data_from_submatrices(Matrix submatrix, Matrix supermatrix, int myRank, int numRanks, int rows_per_rank, int cols_per_rank, int ranks_per_row, int N) {
    ...
    if (myRank != 0){
        MPI_Send(submatrix, elements_per_rank, MPI_DOUBLE, 0, 0, MPI_COMM_WORLD);
    }
    if (myRank == 0) {
        for(int i = 0; i < elements_per_rank; i++){
            int index_in_target = calc_index_supermatrix(myRank, N, ranks_per_row, rows_per_rank, cols_per_rank, i);
