...
void print_temperature(float *m, int N, int M);
...
int main(int argc, char **argv) {
    ...
    for (int t = 0; t < T; t++) {
        ...
        if (submatrix_col_id % 2) {
            MPI_Send(send_left_temps, rows_per_rank, MPI_FLOAT, left, MPI_TAG, MPI_COMM_WORLD);
            MPI_Send(send_right_temps, rows_per_rank, MPI_FLOAT, right, MPI_TAG, MPI_COMM_WORLD);
            MPI_Recv(recv_right_temps, rows_per_rank, MPI_FLOAT, right, MPI_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(recv_left_temps, rows_per_rank, MPI_FLOAT, left, MPI_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
        }
        else {
            MPI_Recv(recv_right_temps, rows_per_rank, MPI_FLOAT, right, MPI_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
            MPI_Recv(recv_left_temps, rows_per_rank, MPI_FLOAT, left, MPI_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
...
void gather_data_from_submatrices(Matrix submatrix, Matrix supermatrix, int my_rank, int num_ranks, int rows_per_rank, int cols_per_rank, int ranks_per_row, int N) {
    ...
    if (my_rank != 0) {
        MPI_Send(submatrix, elements_per_rank, MPI_FLOAT, 0, MPI_TAG, MPI_COMM_WORLD);
    }
    if (my_rank == 0) {
        ...
        for (int i = 1; i < num_ranks; i++){
            MPI_Recv(tmp, elements_per_rank, MPI_FLOAT, i, MPI_TAG, MPI_COMM_WORLD, MPI_STATUS_IGNORE);
